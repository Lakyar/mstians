...
<div class="flex flex-col flex-1">
        ...
        <div class=" mt-12 text-center h-20 ">
            <a href="/tournaments/create" class="items-center text-2xl mt-3 px-6 py-6 rounded-2xl bg-green-500/90 hover:text-black hover:bg-green-600 active:text-white active:bg-black duration-300 text-black font-bold">Create New Tournament +</a>
        </div>
        @unless (count($tournaments) == 0)
            <div class="bg-slate-600 w-full items-center px-10">
                @foreach($tournaments as $tournament)
                    ...
                    <div class="p-2 w-full bg-slate-950 flex my-4 items-center -skew-x-3 rounded-lg shadow-lg shadow-black/30">
                        ...
                        <a href="/tournaments/{{$tournament['id']}}">
                            <img src="/images/{{$tournament->images}}" class="h-28 w-44 mx-2 mr-4" alt="">
                        </a>
                        ...
                        <div class="flex flex-1 flex-col justify-between h-full px-4">
                            ...
                            <div class=" text-lg flex justify-between mt-8 w-full">
                                ...
                                <h5 class="font-thin text-sm my-2 ">
                                    Posted by {{$tournament->user->name}}
                                </h5>
                                <a href="/tournaments/{{$tournament->id}}/edit" class=" bg-slate-800  text-yellow-300 p-1 px-3 rounded-xl text-center">Edit <i class="fa fa-pencil"></i></a>
    
    
    
                                <!-- Delete Button -->
                                <button class="text-red-500  bg-slate-800 p-1 px-3 rounded-xl text-center" onclick="showDeleteConfirmationModal({{$tournament->id}})">Delete <i class="fa fa-trash"></i></button>
                ...
                <div id="deleteConfirmationModal" class="hidden fixed top-1/3 left-[30%] w-[40%] rounded-lg  h-fit bg-black bg-opacity-0 justify-center items-center z-50" >
                    ...
                    <p class="text-xl mb-4 text-red-500 font-semibold">Are you sure you want to delete this tournament? </p>
                        <div class="flex justify-between">
                            ...
                            <form id="deleteTournamentForm" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
    ...
    <script>
        function showDeleteConfirmationModal(tournamentId) {
            document.getElementById('deleteTournamentForm').action = '/tournaments/' + tournamentId;
